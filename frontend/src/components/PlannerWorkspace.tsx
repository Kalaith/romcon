...
export function PlannerWorkspace() {
  ...
  return (
    <div className="w-full px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col gap-4 rounded-[2rem] border border-stone-200 bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.06)] md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-rose-600">Signed in as {workspace.user?.display_name}</p>
          <h1 className="mt-2 font-display text-4xl text-stone-950">RomCon novella planner</h1>
          <p className="mt-2 text-sm font-semibold text-stone-700">{workspace.currentPlan.title || 'Untitled project'}</p>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-stone-600">
            Build the couple first. Then decide whether they can survive a premise built to embarrass them in public.
          </p>
        </div>
        <div className="rounded-full border border-stone-200 bg-stone-50 px-4 py-2 text-sm text-stone-600">{workspace.activeView === 'planner' ? 'Planner workspace' : workspace.workspaceDescription}</div>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="rounded-[1.5rem] border border-stone-200 bg-white p-1.5">
          <div className="flex flex-wrap gap-1">
            <button className={`rounded-full px-4 py-2 text-sm font-semibold ${workspace.activeView === 'planner' ? 'bg-rose-600 text-white' : 'text-stone-700'}`} onClick={() => workspace.setActiveView('planner')}>
              Story Planner
            </button>
            <button className={`rounded-full px-4 py-2 text-sm font-semibold ${workspace.activeView === 'cast' ? 'bg-rose-600 text-white' : 'text-stone-700'}`} onClick={() => workspace.setActiveView('cast')}>
              Cast Library
            </button>
            ...
            {workspace.hasSavedPlan ? (
              <button className={`rounded-full px-4 py-2 text-sm font-semibold ${workspace.activeView === 'summary' ? 'bg-rose-600 text-white' : 'text-stone-700'}`} onClick={() => workspace.setActiveView('summary')}>
                Story Summary
              </button>
            ) : null}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            {isActionsMenuOpen ? (
              <div className="absolute right-0 z-20 mt-2 w-56 rounded-[1.5rem] border border-stone-200 bg-white p-2 shadow-[0_18px_50px_rgba(15,23,42,0.12)]">
                {workspace.user?.is_guest ? (
                  <a className="block rounded-[1rem] px-4 py-3 text-sm font-semibold text-stone-900 hover:bg-stone-50" href={workspace.getLinkAccountUrl()} onClick={() => setIsActionsMenuOpen(false)}>
                    Link Account
                  </a>
                ) : null}
                <button className="block w-full rounded-[1rem] px-4 py-3 text-left text-sm font-semibold text-stone-900 hover:bg-stone-50" onClick={() => { setIsActionsMenuOpen(false); void workspace.handleSaveDraft(); }}>
                  {workspace.currentPlan.id ? 'Update Draft' : 'Save Draft'}
                </button>
                <button className="block w-full rounded-[1rem] px-4 py-3 text-left text-sm font-semibold text-stone-900 hover:bg-stone-50" onClick={() => { setIsActionsMenuOpen(false); void workspace.handleExport('json'); }}>
                  Export JSON
                </button>
                <button className="block w-full rounded-[1rem] px-4 py-3 text-left text-sm font-semibold text-stone-900 hover:bg-stone-50" onClick={() => { setIsActionsMenuOpen(false); void workspace.handleExport('xml'); }}>
                  Export XML
                </button>
                <button className="block w-full rounded-[1rem] px-4 py-3 text-left text-sm font-semibold text-stone-900 hover:bg-stone-50" onClick={() => { setIsActionsMenuOpen(false); workspace.logout(); }}>
                  Sign Out
                </button>
              </div>
            ) : null}
      ...
      {workspace.activeView === 'planner' ? (
        ...
        <div className="mb-6 rounded-[1.5rem] border border-stone-200 bg-white px-5 py-4 text-sm text-stone-600 shadow-[0_10px_35px_rgba(15,23,42,0.05)]">
        ...
        <motion.main initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          {workspace.activeView === 'planner' ? (
            <StoryPlannerView
              ...
              onSaveDraft={() => void workspace.handleSaveDraft()}
